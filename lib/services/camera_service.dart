...
class CameraService {
  ...
  Future<List<CameraGroup>> fetchCameraGroups(String token) async {
    try {
      ...
      try {
        ...
        final debugRes1 = await http.get(debugUri1, headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
        print('DEBUG: TEST /api/camera-statuses Status: ${debugRes1.statusCode}');
        print('DEBUG: TEST /api/camera-statuses Body: ${debugRes1.body}');
        
        final debugUri2 = Uri.parse('$_baseUrl/user/cameras');
        final debugRes2 = await http.get(debugUri2, headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'});
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null) {
          ...
          if (data['groups'] != null && data['groups'] is List) {
            ...
            finalGroups = groupsJson.map((g) => CameraGroup.fromJson(g)).toList();
          ...
          if (data['ungrouped_cameras'] != null && data['ungrouped_cameras'] is List) {
  ...
  Future<Map<String, dynamic>> createGroupApi(String token, String groupName) async {
    try {
      final uri = Uri.parse('$_baseUrl/user/camera-groups');
      ...
      return {'success': false, 'message': body['message'] ?? 'Gagal membuat grup.'};
    } catch (e) {
      return {'success': false, 'message': 'Koneksi error: $e'};
    }
  }

  Future<Map<String, dynamic>> updateGroupApi(String token, String oldGroupName, String newGroupName) async {
    try {
      ...
      final body = json.decode(response.body);
      if (response.statusCode == 200) {
        return {'success': true, 'message': body['message'] ?? 'Berhasil diubah'};
      }
      return {'success': false, 'message': body['message'] ?? 'Gagal mengubah nama grup.'};
    } catch (e) {
      return {'success': false, 'message': 'Error koneksi: $e'};
    }
  }

  Future<Map<String, dynamic>> deleteGroupApi(String token, String groupName) async {
    try {
      ...
      final body = json.decode(response.body);
      if (response.statusCode == 200) {
        return {'success': true, 'message': body['message'] ?? 'Grup berhasil dihapus.'};
      }
      return {'success': false, 'message': body['message'] ?? 'Gagal menghapus grup.'};
    } catch (e) {
      return {'success': false, 'message': 'Error koneksi: $e'};
    }
  }

  Future<Map<String, dynamic>> assignCameraToGroup(String token, String groupName, int cameraId) async {
    try {
      ...
      return {'success': false, 'message': body['message'] ?? 'Gagal menambahkan kamera ke grup.'};
    } catch (e) {
      return {'success': false, 'message': 'Error koneksi: $e'};
    }
  }

  Future<Map<String, dynamic>> removeCameraFromGroup(String token, int cameraId) async {
    try {
      ...
      return {'success': false, 'message': body['message'] ?? 'Gagal menghapus kamera dari grup.'};
    } catch (e) {
      return {'success': false, 'message': 'Error koneksi: $e'};
    }
  }
  ...
  Future<Map<String, dynamic>> updateCamera(
    ...
    String cameraId,
    String name,
    {String? description, String? groupName, int? groupId}
  ) async {
    try {
      ...
      if (groupId != null) bodyData['group_id'] = groupId;
      else if (groupName != null) bodyData['group_name'] = groupName;
  ...
  Future<Map<String, dynamic>> deleteCamera(String token, String id) async {
    try {
      ...
      final body = json.decode(response.body);
      if (response.statusCode == 200) {
        return {'success': true, 'message': body['message'] ?? 'Berhasil dihapus.'};
      } else {
        return {'success': false, 'message': body['message'] ?? 'Gagal menghapus.'};
      }
    } catch (e) {
      return {'success': false, 'message': 'Koneksi error.'};
    }
  ...
  Future<Map<String, dynamic>> addCamera(String token, String deviceId, {String? description}) async {
    try {
      final uri = Uri.parse('$_baseUrl/user/cameras');
      ...
      final body = json.decode(response.body);
      if (response.statusCode == 200 || response.statusCode == 201) {
        return {'success': true, 'message': body['message'] ?? 'Berhasil ditambahkan.'};
      } else {
        return {'success': false, 'message': body['message'] ?? 'Gagal.'};
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getHistoryImages({
    required String token, required String cameraId, required String date,
    required String hour, required String minute, int chunk = 1,
  }) async {
    try {
      final queryParams = {
        'date': date, 'hour': hour, 'minute': minute, 'chunk': chunk.toString(),
      };
      final uri = Uri.parse('$_baseUrl/images/$cameraId/history').replace(queryParameters: queryParams);
      ...
      if (response.statusCode == 200) {
        ...
        if (data['items'] != null) {
          ...
          return items.map((item) {
            ...
            String fullUrl = rawUrl.startsWith('http') ? rawUrl : '$_imageBaseUrl$rawUrl';
            return {
              'id': item['id'],
              'url': fullUrl,
  ...
  Future<Map<String, dynamic>> getHistoryStats(String token, String cameraId, {String? date, String? hour, String? minute}) async {
    try {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (body['success'] == true && body['image_url'] != null) {
          ...
          return imageUrl.startsWith('http') ? imageUrl : '$_imageBaseUrl$imageUrl';
        }
      }
      return null;
    } catch (e) {
